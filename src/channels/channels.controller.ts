...
export class ChannelsController {
    ...
    async createDMChannel(@Param("user_id") user_id: number, @Req() req: any) {
        ...
        if (!creator) {
            throw new UnauthorizedException('You need to be logged in to create a channel.');
        }
    ...
    async updateChannel(@Param("channel_id") channel_id: number, @Req() req: any, @Body() updateData: UpdateChannelDto) {
        const userId = req.user.id;
        if (!userId) {
            throw new UnauthorizedException('You need to be logged in to create a channel.');
        }
    ...
    async manageChannel(@Param("channel_id") channel_id: number, @Req() req: any, @Body() managementData: UserActionDto) {
        const userId = req.user.id;
        if (!userId) {
            throw new UnauthorizedException('You need to be logged in to create a channel.');
        }
        return this.channelsService.manageChannel(channel_id, userId, managementData);
    }

    @UseGuards(AuthGuard('jwt'))
    @Post("/:channel_id/join")
    async joinPublicChannel(@Param("channel_id") channel_id: number, @Req() req: any, @Query("password") password: string) {
        const userId = req.user.id;
        if (!userId) {
            throw new UnauthorizedException('You need to be logged in to create a channel.');
        }
    ...
    @UseGuards(AuthGuard('jwt'))
    @Post("/join")
    async joinPrivateChannel(@Req() req: any, @Query("password") password: string, @Query("name") name: string) {
        const userId = req.user.id;
        if (!userId) {
            throw new UnauthorizedException('You need to be logged in to create a channel.');
        }
